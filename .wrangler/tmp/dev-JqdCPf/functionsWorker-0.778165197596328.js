...
// .wrangler/tmp/pages-kGVvWT/functionsWorker-0.778165197596328.mjs
...
async function onRequestPost(context) {
  ...
  try {
    ...
    switch (language.toLowerCase()) {
      case "english":
        try {
          const response = await env.AI.run("@cf/cloudflare/aura-2-en", { text });
          if (response && response instanceof ReadableStream) {
            ...
            return new Response(
              JSON.stringify({
                ...
                source: "cloudflare-ai-english"
              }),
              { headers: { "Content-Type": "application/json" } }
            );
          }
        } catch (aiError) {
          console.error("Cloudflare AI English TTS failed:", aiError);
        }
        break;
      case "malay":
        try {
          const encodedText = encodeURIComponent(text);
          const googleTTSUrl = `https://translate.google.com/translate_tts?ie=UTF-8&client=tw-ob&tl=ms&q=${encodedText}`;
          ...
          console.error("Google Translate Malay TTS failed:", googleError);
        }
        break;
      case "chinese":
        return new Response(
...
Analyze the word: "${text}"`);
        ...
        try {
          ...
          console.log("Response is not JSON wrapped, will try to parse directly");
        }
        break;
      } catch (e) {
        ...
        if (attempt === MAX_RETRIES) {
          throw new Error(`Failed after ${MAX_RETRIES} attempts. Last error: ${e instanceof Error ? e.message : String(e)}`);
        }
    ...
    try {
      ...
      const sanitizedTranslations = jsonResponse.translations.filter(
        (t) => {
          ...
          return typeof language === "string" && validLangs.includes(language) && typeof word === "string" && word.trim() !== "" && typeof explanation === "string" && explanation.trim() !== "" && typeof example === "string" && example.trim() !== "";
        }
      );
...
var corsHeaders = /* @__PURE__ */ __name2((origin) => ({
...
// .wrangler/tmp/bundle-3deUcM/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-3deUcM/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.778165197596328.js.map
